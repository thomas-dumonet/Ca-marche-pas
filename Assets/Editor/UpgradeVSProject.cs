...
{
    [MenuItem("Tools/Target Framework/Reset Solution Frameworks to v4.5")]
    static void UpgradeSolutions()
    {
        ...
        if (csprojFile != null)
        {
            for (int i = 0; i < csprojFile.Length; i++)
            {
                ...
                if (ReplaceInFile(csprojFile[i], "<TargetFrameworkVersion>v3.5</TargetFrameworkVersion>", "<TargetFrameworkVersion>v4.5</TargetFrameworkVersion>"))
                {
        ...
        if (unityProjFile != null)
        {
            ...
            {
                if (ReplaceInFile(unityProjFile[i], "ToolsVersion=\"3.5\"", "ToolsVersion=\"4.5\""))
                {
                    toolsUpdates.Add(Path.GetFileNameWithoutExtension(unityProjFile[i]));
                }
               
                if (ReplaceInFile(unityProjFile[i], "<TargetFrameworkVersion>v3.5</TargetFrameworkVersion>", "<TargetFrameworkVersion>v4.5</TargetFrameworkVersion>"))
                {
